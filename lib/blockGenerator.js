...
function BlockGenerator(server, prover) {
  ...
  const Certification   = server.lib.Certification;
  ...
  const keyring         = server.lib.keyring;
  const rules           = server.lib.rules;
  const constants       = server.lib.constants;

  this.nextBlock = (manualValues) => generateNextBlock(new NextBlockGenerator(mainContext, conf, dal, logger, Certification, constants, rules), manualValues);
    ...
    for (const obj of txs) {
      ...
      try {
        yield new Promise((resolve, reject) => {
          rules.HELPERS.checkBunchOfTransactions(passingTxs.concat(extractedTX), (err, res) => {
  ...
  const createBlock = (current, joinData, leaveData, updates, revocations, exclusions, transactions, manualValues) => {
    return co(function *() {
      ...
      if (block.number > 0 && Block.statics.getLen(block) > maxLenOfBlock) {
 ...
 * @constructor
 */
function NextBlockGenerator(mainContext, conf, dal, logger, Certification, constants, rules) {
